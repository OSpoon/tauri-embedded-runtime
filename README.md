...
应用会在自己的数据目录中准备所选的 Python 或 Node.js 运行时及对应业务服务依赖，
并启动本地服务供桌面端调用。运行时与项目依赖不会写入宿主机的 PATH、Shell 配置或全局包目录。
...
启动应用后，先选择 Python 或 Node.js，再点击“运行检测”。应用会按顺序检查并准备所选环境：
...
环境准备完成后，点击“打开业务首页”即可调用对应的内置示例服务。服务只监听本机地址，
并由应用负责分配端口、健康检查和关闭回收。

## 主要特性

- 应用私有的 Python 或 Node.js 运行时
...
首次运行需要网络访问运行时制品源和项目包源。运行时文件会写入 Tauri
