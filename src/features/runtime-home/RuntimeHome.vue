    ...
    <ScrollArea class="h-full">
      <div class="mx-auto w-full max-w-5xl px-5 py-5 sm:px-8 sm:py-7 lg:px-10">
        <header class="flex flex-wrap items-start justify-between gap-4">
          <div class="min-w-0">
            <p class="text-[10px] font-semibold uppercase tracking-[0.24em] text-blue-600">Local business workspace</p>
            <h1 class="mt-2 text-2xl font-semibold tracking-[-0.05em] text-zinc-900 sm:text-3xl">业务首页</h1>
            <p class="mt-2 max-w-2xl text-sm leading-6 text-zinc-500">运行环境已经准备完成。你可以从这里调用已启动的本地服务，作为后续真实项目接入的入口。</p>
          </div>
        ...
        <Card class="mt-6 bg-white/80 shadow-sm shadow-zinc-900/[0.03]">
          <CardHeader class="pb-3">
            <div class="flex items-center justify-between gap-3">
              <div>
                <CardTitle class="text-base">本地服务</CardTitle>
                <CardDescription class="mt-1">服务由 App 私有运行时托管，不依赖宿主机环境。</CardDescription>
              </div>
                ...
                <CardHeader class="pb-3">
                  <div class="flex items-start justify-between gap-3">
                    <div class="flex min-w-0 items-center gap-3">
                      ...
                      <div class="min-w-0">
                        <CardTitle class="text-sm">{{ project.display_name }}</CardTitle>
                        <CardDescription class="mt-0.5 text-xs">{{ project.service_label }}</CardDescription>
                      </div>
                    </div>
                    <Badge v-if="serviceFor(project.project_id)?.running" variant="outline" class="shrink-0 text-[10px] font-normal text-emerald-600">运行中</Badge>
                    <Badge v-else variant="outline" class="shrink-0 text-[10px] font-normal text-zinc-400">未启动</Badge>
                  </div>
                </CardHeader>
                <CardContent class="space-y-3">
                  <p class="text-xs leading-5 text-zinc-500">{{ project.framework }} 项目服务<span v-if="serviceFor(project.project_id)?.port"> · 127.0.0.1:{{ serviceFor(project.project_id)?.port }}</span></p>
        ...
        <Card class="mt-4 bg-white/80 shadow-sm shadow-zinc-900/[0.03]">
          <CardHeader class="pb-3">
            <CardTitle class="text-base">最近一次调用</CardTitle>
            <CardDescription class="mt-1">用于快速确认业务服务可以被正常访问。</CardDescription>
          </CardHeader>
          <CardContent>
            <div class="flex min-h-20 items-start gap-3 rounded-lg border border-dashed border-zinc-200 bg-[#fafbfe] p-3">
              ...
              <p class="text-sm leading-6 text-zinc-500">{{ props.message }}</p>
            </div>
